    ...
    def process_frames(self, frames):
        ...
        for frame_idx, frame in enumerate(frames):
            ...
            for lm in results.pose_landmarks.landmark:
                current_landmarks.extend([lm.x, lm.y, lm.z, lm.visibility])
            
            # 유효한 랜드마크인지 확인
            if not any(current_landmarks):
                logging.debug(f"Invalid landmarks in frame {frame_idx}")
                continue
                
            ball_box = detect_basketball_yolo(frame, self.ball_model, device)
            ...
            if ball_box is not None:
                ...
                if right_wrist.visibility > 0.5 or left_wrist.visibility > 0.5:
                    right_wrist_coord = (right_wrist.x * frame.shape[1],
                                    right_wrist.y * frame.shape[0])
                    left_wrist_coord = (left_wrist.x * frame.shape[1],
                                    left_wrist.y * frame.shape[0])
                    
                    right_dist = np.sqrt((right_wrist_coord[0] - ball_center_x)**2 + 
                                    (right_wrist_coord[1] - ball_center_y)**2)
                    left_dist = np.sqrt((left_wrist_coord[0] - ball_center_x)**2 + 
                                    (left_wrist_coord[1] - ball_center_y)**2)
                    min_distance = min(right_dist, left_dist)

                    if min_distance < 50:
                        if not shooting_sequence_found:
                            if pose_buffer:
                                logging.info(f"Adding {len(pose_buffer)} buffered poses")
                                features.extend(pose_buffer)
                            shooting_sequence_found = True
                        features.append(current_landmarks)
                        logging.debug(f"Added frame {frame_idx} to shooting sequence")
                
            if not shooting_sequence_found:
                pose_buffer.append(current_landmarks)
                if len(pose_buffer) > self.frames_per_shot:
                    pose_buffer.pop(0)

        if not features and ball_found and pose_buffer:
            logging.info("Using buffered poses as no shooting sequence was found")
            features.extend(pose_buffer)
        ...
        if np.any(np.isnan(features)) or np.any(np.isinf(features)):
            logging.warning("Invalid values detected in features")
