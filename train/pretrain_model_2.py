    ...
    def process_frames(self, frames):
        ...
        for frame_idx, frame in enumerate(frames):
            ...
            if ball_center is not None:
                for i in range(len(boxes)):
                    if int(cls[i]) == 0:
                        ...
                        distance = np.sqrt((ball_center[0] - person_center[0])**2 + 
                                        (ball_center[1] - person_center[1])**2)
            ...
            if closest_person_box is not None:
                ...
                px2 = min(frame.shape[1], px2); py2 = min(frame.shape[0], py2)
                ...
                if results.pose_landmarks:
                    ...
                    for lm in results.pose_landmarks.landmark:
                        x = lm.x * (px2 - px1) + px1
                        y = lm.y * (py2 - py1) + py1
                        landmarks.extend([x / frame.shape[1], y / frame.shape[0], lm.z, lm.visibility])
        ...
        if ball_found and not shooting_sequence_found and pose_buffer:
            features.extend(pose_buffer)
